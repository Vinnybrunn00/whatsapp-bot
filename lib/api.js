...
class BotApiUtils {
    ...
    async setPhotoGroup(message) {
        try {
            ...
            await this.api.saveLog(this.timeLog, message.author, 'ERROR', err, message.chat.name, ' => setPhotoGroup()')
                                    ...
                                    if (message.to === getId) {
                                        if (isAdmin) {
                                            await bot.promoteParticipant(
                                                message.from, `${participantId.replace('@', '')}@c.us`)
    ...
    async setDemote(participantId, message, bot) {
        try {
            await bot.getGroupAdmins(message.from)
                .then(async demote => {
                    if (demote.includes(`${participantId.replace('@', '')}@c.us`)) {
                        await bot.sendReplyWithMentions(message.from, `• ${participantId} Não está na lista de admins ❗`, message.id)
                        return;
                    }
                            ...
                            if (isAdmin) {
                                for (let i = 0; i < participants.length; i++) {
                                    ...
                                    if (message.to === getId) {
                                        ...
                                        await bot.reply(message.chat.id, '• O bot precisa ser admin para executar este comando ❗', message.id)
                                        return;
                                    }
                                }
                            }
                            await bot.reply(message.chat.id, '• Você precisa ser admin para usar este comando ❗', message.id)
                            return;
                        }
            ...
            await this.api.saveLog(this.timeLog, message.author, 'ERROR', err, message.chat.name, ' => setDemote()')
            return `Erro ao tirar admin de ${participantId}`
        }
    }
    ...
    async sendResolveSticker(mimetype, quotedMsg, type, timer, message, bot) {
        try {
            await bot.sendReplyWithMentions(message.from, this.sendRequestSticker(timer, message.notifyName))
            const decrypt = await decryptMedia(quotedMsg === undefined ? message : quotedMsg)
    ...
    async getLinkGroup(message, bot) {
        ...
        for (let i = 0; i < participants.length; i++) {
            ...
            if (message.to === participants[i]['id']) {
                if (isAdmin) {
                    await bot.getGroupInviteLink(message.chat.id)
                        .then(async link => {
