...
class BotApiUtils {
    ...
    async setPromote(participantId, message, bot) {
        try {
            ...
            await this.saveLog(this.timeLog, message.author, 'ERROR', err, message.chat.name, ' => setPromote()')
    ...
    async setDescription(description, message, bot) {
        ...
        try {
            ...
            await this.saveLog(this.timeLog, message.author, 'ERROR', err, message.chat.name, ' => setDescription()')
    ...
    async sendVoice(text, lang, message, bot) {
        ...
        try {
            ...
            await this.saveLog(this.timeLog, message.author, 'ERROR', err, message.chat.name, ' => sendVoice()')
    ...
    async downloadYt(link, format, message, bot) {
        ...
        if (match && match[2].length == 11) {
            try {
                ...
                await this.saveLog(this.timeLog, message.author, 'ERROR', err, message.chat.name, ' => downloadYt()')
    ...
    async sendResolveSticker(mimetype, quotedMsg, type, timer, message, bot) {
        try {
            ...
            await this.saveLog(this.timeLog, message.author, 'ERROR', err, message.chat.name, ' => sendResolveSticker()')
    ...
    async extractText(message, bot) {
        try {
            ...
            await this.saveLog(this.timeLog, message.author, 'ERROR', err, message.chat.name, ' => extractText()')
