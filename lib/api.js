...
class BotApiUtils {
    ...
    async reactOnOf(outlet) {
        let _flag = outlet === 'desabled' ? false : true
        let _msgReact;
        try {
            let readfile = await fs.readFile(pathReact)
            ...
            onReact.filter(async (value, index, array) => {
                if (_flag === value) {
                    _msgReact = `Reação já está ${value ? 'Ativada' : 'Desativada'}!`
                    return;
                }
                array.splice(index, 1)
                array.push(_flag)
                await fs.writeFile(reactPath, JSON.stringify(array))
                return;
            });
            return _msgReact !== undefined ? _msgReact : _flag ? 'Reação ativada!' : 'Reação desativada'
        } catch (err) {
            throw err
        }
    }
